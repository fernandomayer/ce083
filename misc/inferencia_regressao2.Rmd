...
Os dados do Gapminder mencionados estão disponíveis neste [endereço](http://www.leg.ufpr.br/~fernandomayer/data/pib_gapminder.csv).
...
```{r}
range(dados$expVida[dados$pais == "Chile"])
range(dados$expVida[dados$pais == "Uruguay"])
par(mfrow = c(1,2))
boxplot(dados$expVida[dados$pais == "Chile"], ylim = c(50, 80))
boxplot(dados$expVida[dados$pais == "Uruguay"], ylim = c(50, 80))
par(mfrow = c(1,1))
```
...
med <- mean(dados$expVida)
vari <- var(dados$expVida)
```
...
c(med - tcrit * sqrt(vari/n), med + tcrit * sqrt(vari/n))
```
...
```{r}
t.test(dados$expVida, mu = 60, alternative = "greater")
t.test(dados$expVida, mu = 60, alternative = "less")
## Comparando médias de Chile e Uruguay
t.test(dados$expVida[dados$pais == "Chile"],
       dados$expVida[dados$pais == "Uruguay"], 
       mu = 0)
```
...
o objeto `mod` é da classe `lm`, que possui um método específico para a função `plot()`. Por padrão, ao fazer um gráfico de um objeto da classe `lm`, são gerados quatro gráficos de diagnóstico, como pode ser visto abaixo.
